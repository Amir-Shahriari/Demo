...
_ORACLE_DIR = _ROOT / "demo_agents" / "oracle"
...
async def lifespan(app: FastAPI):
    ...
    total_scenarios = sum(
        len(scale_entry["scenarios"])
        for scale_entry in manifest.values()
    )
    print(f"[startup] Loading {total_scenarios} scenarios across {len(manifest)} scales...")
    ...
    for scale_key, scale_entry in manifest.items():
        for sc_idx, scenario in enumerate(scale_entry["scenarios"]):
            ...
            print(f"[startup]   [{scenario_count + 1}/{total_scenarios}] {cell_id}: rebuilding graph...", flush=True)
            ...
            print(f"[startup]   [{scenario_count + 1}/{total_scenarios}] {cell_id}: loading agents...", flush=True)
            warm_agent = _load_checkpoint(warm_pt, device)
            cold_agent = _load_checkpoint(cold_pt, device)

            print(f"[startup]   [{scenario_count + 1}/{total_scenarios}] {cell_id}: running rollouts...", flush=True)
            wire = _build_wire_format(scale_entry, scenario, warm_agent, cold_agent, g)
            _CELL_CACHE[cell_id] = wire
            ...
            if scale_key == "25x25" and not scenario.get("unsolvable", False):
                ...
                for arm in ("classical_only", "quantum_only"):
                    ck = _ORACLE_DIR / f"seed42_{sc_id}" / arm / "agent.pt"
                    if ck.exists():
                        cell_oracle[arm] = _load_checkpoint(ck, device)
                        print(
                            f"[startup]   [{scenario_count + 1}/{total_scenarios}] {cell_id}:"
                            f" oracle {arm} loaded",
                            flush=True,
                        )
                    else:
                        print(
                            f"[startup]   [{scenario_count + 1}/{total_scenarios}] {cell_id}:"
                            f" WARNING no oracle checkpoint for {arm} at {ck}",
                            flush=True,
                        )

                _ORACLE_AGENTS[cell_id] = cell_oracle
                ...
                for arm, oagent in cell_oracle.items():
                    if arm == "full_pool":
                        _ORACLE_CACHE[f"{cell_id}|{arm}"] = wire
                    else:
            ...
            _CELL_SUMMARIES.append(summary)

            scenario_count += 1
            print(
                f"[startup]   [{scenario_count}/{total_scenarios}] {cell_id}: done "
